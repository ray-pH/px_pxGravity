...
function step_euler(f : diffFun, ti : number, Xi : Float64Array, dt : number) : Float64Array{
    // yn = yi + f * dt
    let Fi = f(ti, Xi);
...
function step_heun(f : diffFun, ti : number, Xi : Float64Array, dt : number) : Float64Array{
    let tn = ti + dt;
    let Fi = f(ti, Xi);
    //  Xp = X + dt*Fi
...
function step_RK4(f : diffFun, ti : number, Xi : Float64Array, dt : number) : Float64Array{
    let tn = ti + dt;
    //  k1 = h * f(T(i)        , Y(i,:)         );
